...
if re.match('.*orrect.*', lemma):
  ...
  for line in lines:
    ...
    elif re.match('.*State.*Message.*', line): rank[90].append(num)
    elif re.match('.*State.*WebSocket.*', line): rank[85].append(num)
    elif re.match('.*State.*Enrolment.*', line): rank[80].append(num)
